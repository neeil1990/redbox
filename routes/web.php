...
//    dd($analysis->getResult());
    ...
    $jayParsedAry = [
        "_token" => "L0Dm7AaIPUydQ15AqRG0IEi2J9Fe83Gjz2mUTjCn",
        "selectedPhrases" => [
            "hleb",
        ],
        "selectedTags" => [
...
//            "title",
    ...
    $phrases = $jayParsedAry['selectedPhrases'];
    $tags = $jayParsedAry['selectedTags'];
    $metaTags = $jayParsedAry['metaTags'];
    ...
    foreach ($result as $tag => $values) {
        foreach ($values as $word => $count) {
            if (($count / $countPhrases) < 3 || $word === 0) {
