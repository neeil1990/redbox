...
Route::get('/test-relevance-method', function () {
    $xml = new SimplifiedXmlFacade(20, 1);
    $xml->setQuery('бинокулярный офтальмоскоп');
    $xmlResponse = $xml->getXMLResponse();

    $relevance = new Relevance('https://riester.su/catalog/nepryamoy-oftalmoskop/');
...
zoon.ru\n\r",
    ...
    foreach ($relevance->pages as $key => $page) {
        $item = $relevance->prepareTfCloud($relevance->separateText($page['html'] . ' ' . $page['linkText']));
        if ($key == 'https://stormoff.ru/catalog/oftalmologiya/diagnostika/oftalmoskopy/binokulyarnye-oftalmoskopy/') {
            dd($item);
        }
        $tfCompClouds[$key] = $item;
    }
    dd($tfCompClouds);
});
